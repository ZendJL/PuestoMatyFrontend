export default function MermaTabla({
  itemsMerma, formatMoney, cambiarCantidadItem, quitarItem, pageSize, setPageSize
}) {
  return (
    <div className="card shadow-sm mb-3">
      <div className="card-header d-flex justify-content-between align-items-center bg-body-tertiary border-bottom">
        <h6 className="mb-0 fw-semibold">
          🛒 Productos en merma <span className="badge bg-primary">{itemsMerma.length}</span>
        </h6>
        <select className="form-select form-select-sm" style={{width: '90px'}} 
                value={pageSize} onChange={e => setPageSize(parseInt(e.target.value))}>
        ...
        <div className="table-responsive">
          <table className="table table-hover table-sm mb-0">
            ...
            <tbody className="table-group-divider">
              {itemsMerma.slice(0, pageSize).map((item) => {
